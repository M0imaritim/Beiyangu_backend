...
class UserAdmin(BaseUserAdmin):
    ...
    def user_stats(self, obj):
        ...
        stats = []
        if request_count > 0:
            stats.append(f'{request_count} request{"s" if request_count != 1 else ""}')
        if bid_count > 0:
    ...
    def permissions_display(self, obj):
        ...
        if obj.is_superuser:
            badges.append('<span style="background: red; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">SUPER</span>')
        elif obj.is_staff:
            badges.append('<span style="background: blue; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">STAFF</span>')
        
        if not obj.is_active:
            badges.append('<span style="background: gray; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">INACTIVE</span>')
