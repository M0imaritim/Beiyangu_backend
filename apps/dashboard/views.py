...
def buyer_dashboard(request):
    ...
    # Get statistics
    stats = {
        'total_requests': Request.objects.filter(buyer=user, is_deleted=False).count(),
        'open_requests': Request.objects.filter(
            buyer=user, status='open', is_deleted=False
        ).count(),
...
def seller_dashboard(request):
    ...
    # Get statistics
    stats = {
        'total_bids': Bid.objects.filter(seller=user, is_deleted=False).count(),
        'accepted_bids': Bid.objects.filter(
            seller=user, is_accepted=True, is_deleted=False
        ).count(),
        ...
        'pending_earnings': Bid.objects.filter(
            seller=user, is_accepted=True, is_deleted=False,
            request__status__in=['accepted', 'delivered']
        ).aggregate(
            total=Sum('amount')
        )['total'] or 0,
    }
    ...
    available_requests = Request.objects.select_related('buyer', 'category').filter(
        status='open',
        is_active=True,
        is_deleted=False
    ).exclude(
    ...
    return Response({
        ...
        'data': {
            ...
            'available_requests': RequestSerializer(available_requests, many=True).data
        }
    })